// `mcpverify automation agent delete <id> --yes`

import { parseArgs } from 'node:util';
import { McpVerifyClient } from '../../client-class.js';

export async function agentDeleteHandler(args: string[]): Promise<void> {
  ...
  if (values.help) {
    process.stdout.write(`mcpverify automation agent delete <agentId> — delete an LLM-as-judge agent
...
`);
  ...
  if (!id) { console.error('Error: agent id is required'); process.exit(2); }
  if (!values.yes) { console.error(`Refusing to delete agent ${id} without --yes`); process.exit(2); }
  ...
  try {
    await client.automation.agents.delete(id);
    process.stdout.write(`Deleted agent ${id}\n`);
  } catch (err) {
    ...
    if (e.status === 404) { console.error(`Error: agent ${id} not found`); process.exit(3); }
